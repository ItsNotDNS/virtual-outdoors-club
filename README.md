...
If you wish to host the frontend through Nginx, you would need to setup the configuration to host the index.html file generated by the dist folder. You would need to move the dist folder built with `npm run-script build` into the site folder and sent up the location similar to the thread indicated below (2nd link). Future configuration can be implemented with the SSL certification and an automation worker to handle recertification.

[Digital Ocean - Nginx](https://www.digitalocean.com/community/tutorials/how-to-install-nginx-on-ubuntu-16-04)

[Digital Ocean - SSL with Nginx](https://www.digitalocean.com/community/tutorials/how-to-secure-nginx-with-let-s-encrypt-on-ubuntu-16-04)

[Nginx Path Redirection in React](https://stackoverflow.com/questions/49251167/react-router-routes-not-working-on-nginx-create-react-app)

When using Nginx, you need to add another step in making the production build for the front-end. The `dist` folder needs to be moved into the site folder of your choice in the Nginx ```/var/www/``` directory.
...
- To run the Email worker in the background to allow automated emails to be sent out for reservations
  ...
  ```python3 manage.py createworkers```
  
  If you had a worker created previously and need to re-create the worker run
